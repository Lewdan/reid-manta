...
class Siamese(BaseModel):
    def __init__(self, backend, input_shape, frontend, embedding_size,
                 connect_layer=-1, train_from_layer=0, distance='l2', loss_func='contrastive', show_summary=True,
                 weights='imagenet'):
    ...
    def compute_dist(self, images, labels, sample_size=None):
        ...
        print(
            'Number of pairs in evaluation {}, number of positive {}'.format(len(actual_issame), np.sum(actual_issame)))
